...
; -> We enter an infinite loop since new-if uses applicative order evaluation instead of normal order evaluation, and that means we need to evaluate sqr-iter before the predicate entering the infinitite loop.